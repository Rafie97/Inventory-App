    ...
    await hebRef.onSnapshot((snap) => {
      ...
      snap.forEach((doc) => {
        ...
        this.setState({
          ...
          backSearches: [...this.state.backSearches, item],
        });
      });
    });

    mainReducer({ products: this.state.cards }, { type: "SET_PRODUCTS" });
  }
      ...
      <div style={{ float: "left", marginLeft: 20 }}>
        <div style={{ marginBottom: 40, marginTop: 40, textAlign: "start" }}>
          <input
            placeholder="Search"
            type="text"
            onChange={(val) => this.searchItems(val.target.value)}
        ...
        <div className="cards">
          {!this.state.cards.length ? (
            ...
            this.state.backSearches.map((card) => (
              <CustomCard
                data={card}
